...
def list_open_issues(limit: int | str = 10):
    ...
    if not issues:
        return {"status": "success", "count": 0, "message": "No open Github issues found in the repository"}
    
    return [
        {"issue_number": item["number"], "issue_title": item["title"], "labels": [label["name"] for label in item["labels"]]} for item in issues
    ]
